...
async function main() {
 ...
 // Mounted at /data/events.csv
 const parser = fs.createReadStream("/data/events.csv").pipe(
  parse({
 ...
 for await (const row of parser) {
  ...
  all.push({
   ...
   // Add a date field for easier querying if needed, but keeping original schema for now
  });
 }
